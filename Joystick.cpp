...
/*
  Modify version for SEGA Megadrive/Genesis Mini
  Oh!Ishi (c) 2019
*/
	...
	bool includeSteering)
{
	...
	// Button Calculations
	uint8_t buttonsInLastByte = (_buttonCount + 4) % 8;
	uint8_t buttonPaddingBits = 0;
	if (buttonsInLastByte > 0)
	{
		buttonPaddingBits = 8 - buttonsInLastByte + 8;
	}
	...
	if (buttonCount > 0) {
		_buttonValuesArraySize = (_buttonCount + 4) / 8;
		if (((_buttonCount + 4) % 8) > 0) {
...
{
	return buildAndSet8BitValue(includeAxis, axisValue, axisMinimum, axisMaximum, JOYSTICK_AXIS_MINIMUM, JOYSTICK_AXIS_MAXIMUM, dataLocation);
}
...
void Joystick_::sendState()
{
	...
	{
		data[index] = _buttonValues[index - (_hidReportSize - 3)];
	}
